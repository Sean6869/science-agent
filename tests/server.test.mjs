...
test('course catalog exposes three simulations and matching knowledge prompts',async()=>{const p=await(await fetch(base+'/api/config')).json();assert.equal(p.defaultLessonId,'bending-light');assert.deepEqual(p.lessons.map(l=>l.id),['bending-light','geometric-optics-basics','energy-skate-park']);for(const [i,l] of p.lessons.entries()){assert.equal(l.number,i+1);assert.equal(l.questions.length,3);assert.ok(l.questions.every(question=>question.endsWith('？')));assert.equal(l.url,`https://phet.colorado.edu/sims/html/${l.id}/latest/${l.id}_${i===2?'all.html?locale=zh_CN':'zh_CN.html'}`);}});
