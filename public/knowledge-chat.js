...
export function createKnowledgeChat(){
 ...
 function submitText(value){
  ...
  const turn={id:crypto.randomUUID(),text,history:state.messages.filter(m=>m.role==='user'||m.role==='agent').slice(-8).map(({role,text})=>({role,text}))};
