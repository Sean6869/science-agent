...
function deleteButton(label,warning,url,refresh){return button(label,async event=>{if(!window.confirm(warning+'\n此操作不可撤销，确定继续吗？'))return;const b=event.currentTarget;b.disabled=true;try{await api(url,{},'DELETE');await refresh();}catch(err){alert(err.message);b.disabled=false;}},'school-link danger');}
function rowActions(...items){const wrap=e('span',undefined,'row-actions');wrap.append(...items);return wrap;}
...
async function studentTests(){
 ...
 const complete=q=>completed.find(s=>s.quizId===q.id&&s.version===q.version);const next=quizzes.find(q=>!complete(q));
 if(!next){await studentEntry();return;}
 const card=e('section',undefined,'school-card quiz-card');card.append(header(next.title,`入门测验 ${quizzes.indexOf(next)+1} / ${quizzes.length}`));
 ...
 const submit=e('button','提交测验','school-button primary');submit.type='submit';form.append(progress,submit);card.append(form);screen.append(card);
...
async function studentEntry(group){
 ...
 const form=e('form');const input=field(form,'小组码','code');input.autocomplete='off';input.maxLength=8;const submit=e('button','进入实验室','school-button primary');submit.type='submit';submit.disabled=group.status!=='code_required';input.disabled=submit.disabled;
 form.append(submit,button('刷新状态',()=>studentEntry().catch(showError),'school-link'));card.append(status,form);screen.append(card);
 form.onsubmit=async event=>{event.preventDefault();submit.disabled=true;try{const result=await api('/api/groups/join',{code:input.value});await startWorkspace(result.group);}catch(err){status.textContent=err.message;submit.disabled=false;}};
}
...
async function teacher(){
 screen.replaceChildren(accountBar());const card=e('section',undefined,'school-card teacher-card');card.append(header(identity.role==='admin'?'管理员管理':'教师管理'));
 ...
 for(const [key,label] of tabs){const b=button(label,()=>render(key));b.dataset.tab=key;nav.append(b);}nav.append(button('刷新',()=>render(selected),'school-link'));
 {const {classes}=await api('/api/teacher/classes');if(!classes.some(c=>c.id===selectedClass))selectedClass='';const select=classSelect(classes,true);select.value=selectedClass;select.onchange=()=>{selectedClass=select.value;void render(selected);};card.append(select);}
 card.append(nav,panel);screen.append(card);await render('students');
}
function classSelect(classes,all=false){const select=e('select');select.name='classId';select.setAttribute('aria-label','班级');const blank=e('option',all?'全部班级':'请选择班级');blank.value='';select.append(blank);for(const c of classes){const option=e('option',c.name+(c.teacherName?` · ${c.teacherName}（${c.teacherUsername}）`:''));option.value=c.id;select.append(option);}return select;}
async function teachersPanel(panel){
 const {teachers}=await api('/api/admin/teachers');panel.replaceChildren();const editor=e('section',undefined,'student-editor');editor.hidden=true;
 function edit(t){editor.hidden=false;editor.replaceChildren(e('h2','编辑教师'));const form=e('form',undefined,'student-form');for(const [label,key] of [['姓名','name'],['账号','username']])field(form,label,key,'text',t[key]);field(form,'新密码（留空则不修改）','password','password','',false);const active=e('input');active.type='checkbox';active.checked=t.active;const activeLabel=e('label','启用账号');activeLabel.prepend(active);form.append(activeLabel);const submit=e('button','保存修改','school-button primary');submit.type='submit';const status=message('');form.append(submit,button('取消',()=>editor.hidden=true,'school-link'));editor.append(form,status);form.onsubmit=async event=>{event.preventDefault();submit.disabled=true;try{await api('/api/admin/teachers/'+t.id,{...Object.fromEntries(new FormData(form)),active:active.checked},'PUT');await teachersPanel(panel);}catch(err){status.textContent=err.message;submit.disabled=false;}};}
 const passwordCell=t=>{if(!t.password)return e('span','未保存');const wrap=e('span',undefined,'password-cell'),value=e('span','••••••••');let visible=false;wrap.append(value,button('显示',event=>{visible=!visible;value.textContent=visible?t.password:'••••••••';event.currentTarget.textContent=visible?'隐藏':'显示';},'school-link'));return wrap;};
 await classOwnership(panel);panel.append(editor,table([['姓名','name'],['班级','className'],['账号','username'],['密码',passwordCell],['状态',t=>t.active?'启用':'停用'],['操作',t=>rowActions(button('编辑 / 重置密码',()=>edit(t),'school-link'),deleteButton('删除',`将删除教师“${t.name}”的账号，教师将无法登录。班级、学生账号、成绩和学生对话记录仍由管理员保留。`,'/api/admin/teachers/'+t.id,()=>teachersPanel(panel)))]],teachers));
}
async function classOwnership(panel){
 const [{classes},{teachers}]=await Promise.all([api('/api/teacher/classes'),api('/api/admin/teachers')]);
 const section=e('section',undefined,'student-editor');section.append(e('h2','班级归属'));
 for(const c of classes){const row=e('div',undefined,'school-tabs'),select=e('select');select.setAttribute('aria-label',c.name+'的教师');const empty=e('option','管理员管理');empty.value='';select.append(empty);for(const t of teachers.filter(t=>t.active)){const option=e('option',t.name+'（'+t.username+'）');option.value=t.id;option.selected=t.username===c.teacherUsername;select.append(option);}row.append(e('span',c.name+' · '+(c.teacherName||'管理员管理')),select,button('分配班级',async()=>{if(!select.value)return;if(!confirm('将“'+c.name+'”及其学生、成绩和对话记录分配给所选教师？'))return;try{await api('/api/admin/classes/assign',{classId:c.id,teacherId:select.value});await teachersPanel(panel);}catch(error){section.append(message(error.message,true));}}));section.append(row);}
 panel.append(section);
}

async function studentsPanel(panel){
 const [{students},{classes}]=await Promise.all([api(scoped('/api/teacher/students')),api('/api/teacher/classes')]);panel.replaceChildren();const editor=e('section',undefined,'student-editor');
 function edit(student){editor.replaceChildren(e('h2',student?'编辑学生账号':'创建学生账号'));const form=e('form',undefined,'student-form');field(form,'姓名','name','text',student?.name||'');field(form,'班级','className','text',student?.className||classes.find(c=>c.id===selectedClass)?.name||'');field(form,'年龄','age','number',student?.age||'',false);const label=e('label',undefined,'school-field');label.append(e('span','性别'));const select=e('select');select.name='gender';for(const value of ['未填写','男','女']){const option=e('option',value);option.value=value;option.selected=student?.gender===value;select.append(option);}label.append(select);form.append(label);const username=field(form,'账号','username','text',student?.username||'');username.pattern='[a-zA-Z0-9_.-]{3,40}';field(form,student?'新密码（留空则不修改）':'初始密码（至少8位）','password','password','',!student);
 ...
 const submit=e('button',student?'保存修改':'创建账号','school-button primary');submit.type='submit';const status=message('');const controls=e('div',undefined,'form-actions');controls.append(submit);if(student)controls.append(button('取消编辑',()=>edit(null),'school-link'));form.append(controls);editor.append(form,status);
 form.onsubmit=async event=>{event.preventDefault();submit.disabled=true;try{const values={...Object.fromEntries(new FormData(form)),active:active.checked};const originalClass=student?{id:student.classId,name:student.className}:classes.find(c=>c.id===selectedClass);if(originalClass&&values.className.trim()===originalClass.name)values.classId=originalClass.id;await api(student?`/api/teacher/students/${student.id}`:'/api/teacher/students',values,student?'PUT':'POST');await teacher();}catch(err){status.textContent=err.message;status.className='school-error';submit.disabled=false;}};
 }
 const batch=e('section',undefined,'student-editor');batch.append(e('h2','批量导入学生'));const upload=e('input');upload.type='file';upload.accept='.xlsx';upload.setAttribute('aria-label','学生Excel文件');const status=message(''),actions=e('div',undefined,'school-tabs');const importButton=button('导入学生',async()=>{const file=upload.files[0];if(!file){status.textContent='请选择Excel文件';return;}if(file.size>1024*1024){status.textContent='文件不能超过1MB';return;}importButton.disabled=true;try{const data=await new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(reader.result.split(',')[1]);reader.onerror=reject;reader.readAsDataURL(file);});const result=await api('/api/teacher/import',{data});await teacher();screen.prepend(message(result.alreadyImported?'此文件已导入，可直接导出账号。':`已创建 ${result.created} 个学生账号`));}catch(err){status.textContent=err.message;importButton.disabled=false;}});const template=e('a','下载模板','school-link');template.href='/student-template.xlsx';template.download='student_information.xlsx';const download=e('a','导出账号密码','school-button');download.href=scoped('/api/teacher/accounts.xlsx');const fileRow=e('div',undefined,'school-tabs');fileRow.append(upload,template);actions.append(importButton,download);batch.append(fileRow,actions,status);
 edit(null);panel.append(batch,editor,e('h2',`学生列表 · ${students.length}人`),table([['姓名','name'],['性别','gender'],['年龄','age'],['班级','className'],['账号','username'],['状态',s=>s.active?'启用':'停用'],['操作',s=>rowActions(button('编辑 / 重置密码',()=>{edit(s);editor.scrollIntoView({behavior:'smooth'});},'school-link'),deleteButton('删除',`将删除学生“${s.name}”的账号、全部测验成绩和AI对话记录，并使本班分组及小组码失效。`,'/api/teacher/students/'+s.id,()=>studentsPanel(panel)))]],students));
}
function setExport(node){const nav=screen.querySelector('.management-nav');nav.querySelector('[data-export]')?.remove();node.dataset.export='true';nav.append(node);}
...
async function conversationsPanel(panel){
 const {students}=await api(scoped('/api/teacher/students'));panel.replaceChildren();const filters=e('div',undefined,'school-tabs'),select=e('select');select.setAttribute('aria-label','筛选学生');const all=e('option','全部学生');all.value='';select.append(all);for(const s of students){const option=e('option',`${s.className} · ${s.name}（${s.username}）`);option.value=s.id;select.append(option);}const list=e('section'),pager=e('div',undefined,'school-tabs');let page=0;const exportArea=e('span');filters.append(select);setExport(exportArea);panel.append(filters,list,pager);
 const load=async()=>{list.replaceChildren(message('正在加载…'));pager.replaceChildren();exportArea.replaceChildren(exportLink('conversations',select.value));try{const {conversations}=await api(scoped(`/api/teacher/conversations?student=${encodeURIComponent(select.value)}&page=${page}`));list.replaceChildren();if(!conversations.length)list.append(message('暂无记录'));for(const row of conversations){const item=e('article',undefined,'conversation-record');item.append(e('h3',`${row.name} · ${row.className} · ${row.agent==='knowledge'?'知识答疑':'元认知支架'}`),e('small',`${time(row.createdAt)} · ${row.lessonId} ${row.stage?`· 环节${row.stage}`:''} · ${row.source==='model'?'DeepSeek在线回答':row.source==='fallback'?'离线备用回复':row.source==='rule'?'流程提示':row.status}`),e('h4','学生'),e('p',row.userText),e('h4','AI'),e('p',row.reply||'未收到回复'),deleteButton('删除对话',`将删除“${row.name}”这条提问及对应的AI回答。`,'/api/teacher/conversations/'+row.id,()=>load()));list.append(item);}const prev=button('上一页',()=>{page--;void load();});prev.disabled=page===0;const next=button('下一页',()=>{page++;void load();});next.disabled=conversations.length<100;pager.append(prev,e('span',`第${page+1}页`),next);}catch(err){list.replaceChildren(message(err.message,true));}};
 select.onchange=()=>{page=0;void load();};await load();
}
async function startWorkspace(group){window.schoolGroup=group;screen.hidden=true;document.body.classList.add('workspace-ready');window.schoolUser=identity;document.querySelector('.app').hidden=false;const bar=accountBar();bar.classList.add('workspace-account');bar.append(e('span',`第 ${group.number} 组 · ${group.members.join('、')}`));document.querySelector('.panel-head').after(bar);await import('./app.js');}
