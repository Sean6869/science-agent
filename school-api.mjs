...
export function createSchoolApi(store,{json,body}){
 ...
 return async function schoolApi(req,res,url){
  ...
  if(path.startsWith('/api/')&&req.method!=='GET'){
   ...
   if(path!=='/api/auth/login'&&(!session||req.headers['x-csrf-token']!==session.csrf)){json(res,session?403:401,{message:'登录已过期，请重新登录'});return true;}
  }
  ...
  if(path==='/api/me'&&req.method==='GET'){json(res,200,session?{...session,scores:store.scores(session.user.id),ready:session.user.role==='teacher'||store.ready(session.user.id)}:{user:null});return true;}
  ...
  if(path.startsWith('/api/teacher/')){
   if(session.user.role!=='teacher'){json(res,403,{message:'仅教师可以访问'});return true;}
   if(path==='/api/teacher/students'){
    if(req.method==='GET'){json(res,200,{students:store.students()});return true;}
    if(req.method==='POST'){try{json(res,201,{student:await store.addStudent(await body(req))});}catch(e){json(res,400,{message:e.message});}return true;}
   }
   if(path.startsWith('/api/teacher/students/')&&req.method==='PUT'){try{json(res,200,{student:await store.editStudent(path.split('/').at(-1),await body(req))});}catch(e){json(res,400,{message:e.message});}return true;}
   if(path==='/api/teacher/scores'&&req.method==='GET'){json(res,200,{scores:store.allScores()});return true;}
   if(path==='/api/teacher/conversations'&&req.method==='GET'){const page=Math.max(0,Math.min(100000,Number(url.searchParams.get('page'))||0));json(res,200,{conversations:store.conversations(url.searchParams.get('student')||'',100,page*100),page});return true;}
   if(path==='/api/teacher/export'&&req.method==='GET'){
    ...
    if(type==='scores')res.end(csv([...common,['测验','quizId'],['试卷版本','version'],['分数（满分100）','score'],['答对题数','correct'],['题目数','total'],['作答','answers'],['提交时间（UTC）','createdAt']],store.allScores()));
    else{
     ...
     res.write(csv(columns,[]));for(let offset=0;;offset+=500){const rows=store.conversations(url.searchParams.get('student')||'',500,offset);if(!rows.length)break;res.write('\r\n'+csv(columns,rows).split('\r\n').slice(1).join('\r\n'));}res.end();
    }return true;
   }
  }
